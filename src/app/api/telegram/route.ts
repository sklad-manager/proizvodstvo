  ...
  if (update.callback_query) {
    ...
    try {
      ...
      // --- Анализ ИИ ---
        ...
        const prompt = `
Проанализируй фото чека. Определи ВСЕ позиции (товары/услуги).
Также найди на фото обведённую в кружок цифру — это номер чека, написанный от руки.

Верни ТОЛЬКО JSON:
{
  "receiptNum": число или null (обведённая цифра на фото, если есть),
  "items": [
    { "description": "название", "amount": число, "category": "...", "paymentMethod": "Ф1" }
  ]
}

Категории: materials (сырье), maintenance (ремонт), salary (зарплаты), rent (аренда), small (мелкие траты), other (прочее).
Если не можешь разобрать, верни хотя бы общую сумму как одну позицию.
`;
